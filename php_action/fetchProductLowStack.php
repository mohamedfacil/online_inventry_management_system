...
require_once 'core.php';
	...
	$sql = "SELECT product.product_id, product.product_name, product.product_image, product.brand_id,
		...
		WHERE (product.status = 1 AND product.quantity = 0) OR (product.active = 2 AND product.status = 1)";
...
if($result->num_rows > 0) { 
 ...
 while($row = $result->fetch_array()) {
 	...
 	if($row[7] == 1) {
 		// activate member
 		$active = "<label class='label label-success'>In Stock</label>";
 	} else {
 		// deactivate member
 		$active = "<label class='label label-danger'>Out of Stock</label>";
 	} // /else
	...
	$checkbox ="<input type=\"checkbox\" value='".$productId."' >";
 	$output['data'][] = array( 	
		$checkbox,	
 		// image
 		$productImage,
